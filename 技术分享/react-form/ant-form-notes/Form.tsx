...
export default class Form extends React.Component<FormProps, any> {
  ...
  static create = function <TOwnProps>(options: FormCreateOption<TOwnProps> = {}): ComponentDecorator<TOwnProps> {
    ...
    const formWrapper = createDOMForm({
      fieldNameProp: 'id', // 设置表单项存储name的键
      ...options,
    ...
    return (Component) => formWrapper(createReactClass({
      ...
      // 子组件可通过 this.context.form 访问相关属性
      getChildContext() {
        return {
      ...
      // 保存默认 getFieldProps 方法
      componentWillMount() {
        this.__getFieldProps = this.props.form.getFieldProps;
      },

      // 推荐使用 getFieldDecorator 初始化表单项
  ...
  shouldComponentUpdate(...args) {
    // 性能优化，比较新旧props和state,减少虚拟dom的diff
